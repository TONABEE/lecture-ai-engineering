...
def display_home_page():
  ...
  # テキスト入力
  st.subheader("ルール確認")
  name = st.text_input("ゲスト", key="question1_input", height=100, value=st.session_state.get("current1_question", ""))
  st.write(f"こんにちは、{name}さん！")
  st.header("貢献度")
  tab1, tab2 = st.tabs(["戦績の推移", "エビデンス作成数"])
  with tab1:
    st.subheader("戦績の推移")
    chart_data = pd.DataFrame(
        np.random.randn(20, 3),
        columns=['千秋', '歩希', 'すみか','ショート'])
    st.line_chart(chart_data)
  with tab2:
    st.subheader("エビデンス作成数")
    chart_data = pd.DataFrame({
        'カテゴリ': ['千秋', '歩希', 'すみか','ショート'],
        '値': [10, 25, 15, 30]
    }).set_index('カテゴリ')
...
# --- チャットページのUI ---
def display_pd_page(pipe):
    """チャットページのUIを表示する"""
    st.subheader("atack/protectを入力してください")
    user_question = st.text_area("???", key="question_input", height=100, value=st.session_state.get("current_question", ""))
